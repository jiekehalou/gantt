...
// var DIM_ACREG_INDEX = 10;//acReg机号
...
function makeOption() {
    return {
        ...
        animation: false,
        color: ['#37A2DA', '#00DA88', '#67E0E3', '#9FE6B8', '#FFDB5C', '#FF9F7F'],
        toolbox: {
            left: 20,
            top: 0,
            itemSize: 20,
        ...
        xAxis: {
            ...
            position: 'top',
            splitNumber: 36,
            minInterval:3600000,//一个小时
            interval:3600000,//一个小时
            min: 1559908800000,//当天-4小时
            max: 1560024000000,//当天+4小时
            splitLine: {
                lineStyle: {
                    color: ['#E9EDFF']
                }
        ...
        yAxis: {
            ...
            // scale: 36 / HEIGHT_RATIO,
            // splitNumber:36/HEIGHT_RATIO,
            interval: 6,
            min: 0,
        ...
        series: [
            {
                ...
                encode: {
                    ...
                    // 这里分别是维度 0、1、2、3、4。
                    //[12, 44, 55, 66, 2],  
                    //此处为["Parking Apron Index","Arrival Time","Departure Time""Flight Number","VIP","Arrival Company","Departure Company","Arrival Line","Departure Line","Report Time"]
                    x: [DIM_TIME_ARRIVAL, DIM_TIME_DEPARTURE],
                    y: DIM_CATEGORY_INDEX,
                    tooltip: [DIM_CATEGORY_INDEX, DIM_TIME_ARRIVAL, DIM_TIME_DEPARTURE]
                },
                data: _rawData.flight.data.plan
            },
            {
                id: 'flightDataActual',
                type: 'custom',
                ...
                data: _rawData.flight.data.Actual
            },
            {
                type: 'custom',
...
function renderGanttItem(params, api) {
    ...
    var timeArrival = api.coord([api.value(DIM_TIME_ARRIVAL), categoryIndex]);//起飞点经度纬度坐标
    var timeDeparture = api.coord([api.value(DIM_TIME_DEPARTURE), categoryIndex]);//到达点经度纬度坐标
    // debugger
    //计算绘图范围
    ...
    var Arrival = (api.value(7) + '').substr(0, 4);//起飞机场
    var Departure = (api.value(8) + '').substr(0, 4);//落地机场
    // console.log(' echarts.format.getTextRect(flightNumber).width', echarts.format.getTextRect(flightNumber).width)
    // console.log(' barLength', barLength)
    var flightNumberWidth = echarts.format.getTextRect(flightNumber).width;//文字字体的宽度；
    var ArrivalWidth = echarts.format.getTextRect(Arrival).width;//文字字体的宽度；
    var ArrivalHeight = echarts.format.getTextRect(Arrival).height;//文字字体的高度；
    ...
    let ArrivalText = {
        type: 'text',
        position: [0, Math.abs(ArrivalHeight - barHeight) / 2],
        style: {
            text: Arrival,
            x: x - ArrivalWidth - TEXTGAP, y: y,
...
function initDrag() {
    ...
    function updateRawData() {
        var flightData = _rawData.flight.data.plan;//只有计划航班能拖动
...
function initChart(rawData, dom, cb) {
    _rawData = rawData;
    myChart = echarts.init(dom);
    myChart.setOption(makeOption());
    // debugger;
    // myChart.getZr()获取zrender 实例；
    initDrag();
